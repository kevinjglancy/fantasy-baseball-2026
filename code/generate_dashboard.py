...
def zscore_players(players, cats, lower_better=set(), min_key=None, min_val=0):
...
pitchers=zscore_players(pitchers,PIT_CATS,lower_better={'era','whip'},min_key='ip_dec',min_val=10)
all_players=batters+pitchers

all_batters_ranked=zscore_players([dict(b) for b in batters],BAT_CATS,min_key='ab',min_val=50)
all_pitchers_ranked=zscore_players([dict(p) for p in pitchers],PIT_CATS,lower_better={'era','whip'},min_key='ip_dec',min_val=10)
